      ...
      <div key={app.id} className={styles.card}>
        ...
        {actionId === app.id ? (
          <div className={styles.actionBlock}>
            ...
            {isSupervisorApp && (
              <p className={styles.interestedHint}>
                Перед назначением свяжитесь с научным руководителем и обсудите проект — его контакты указаны выше в карточке. Назначение сразу закрепит {app.supervisor?.user.name || "этого руководителя"} вашим научным руководителем. Заявки на руководство от других научных руководителей по этому проекту будут автоматически отклонены (заявки студентов в команду — останутся).
              </p>
            )}
            <div className={styles.actionButtons}>
              <button
                onClick={() => {
                  if (isSupervisorApp) {
                    ...
                    if (!confirm(`Назначить ${name} научным руководителем проекта?\n\nВы должны были предварительно связаться и пообщаться с ним (контакты — в карточке заявки).\n\nПосле назначения заявки на руководство от других научных руководителей по этому проекту будут автоматически отклонены.`)) return;
                  }
