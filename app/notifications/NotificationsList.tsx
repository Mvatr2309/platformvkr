...
import type { NotificationSpace } from "@/lib/notification-space";
...
export default function NotificationsList({ space }: { space: NotificationSpace }) {
  ...
  const fetchNotifications = useCallback(async () => {
    try {
      const res = await fetch(`/api/notifications?limit=50&space=${space}`);
      if (res.ok) {
  ...
  useEffect(() => {
    ...
    const t = setTimeout(() => {
      fetch("/api/notifications", {
        ...
        body: JSON.stringify({ all: true, space }),
      }).then(() => {
  ...
  async function handleClick(notif: Notification) {
    ...
    if (!notif.read) {
      fetch("/api/notifications", {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ ids: [notif.id], space }),
      });
      ...
      <div className={styles.container}>
        <div className={styles.headerRow}>
          <h1 className={styles.title}>
            {space === "expert" ? "Уведомления экспертной трубы" : "Уведомления"}
          </h1>
          {unreadCount > 0 && (
        ...
        {loading ? (
          ...
          <p className={styles.empty}>
            {space === "expert"
              ? "В экспертной трубе уведомлений нет"
              : "Нет уведомлений"}
          </p>
        ) : (
          <>
