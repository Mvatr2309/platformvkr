...
// POST /api/admin/matching — назначить НР на проект (04.02, 04.03)
export async function POST(request: NextRequest) {
  ...
  await prisma.activity.create({
    data: {
      projectId,
      action: `Назначен руководитель: ${supervisor.user.name}`,
    },
  });

  // 04.03 — уведомление руководителю
  try {
    await sendMail({
      to: supervisor.user.email,
      subject: "Вы назначены руководителем проекта — Платформа ВКР",
      html: `
        <div style="font-family: 'Montserrat', Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 32px;">
          <h2 style="color: #003092;">Вы назначены руководителем проекта</h2>
          <p style="color: #333;">Уважаемый(ая) <strong>${supervisor.user.name}</strong>,</p>
          <p style="color: #555;">Вы были назначены руководителем проекта «${project.title}».</p>
